...
abstract class Sitengine_Newsletter_Backend_Campaigns_Attachments_AssignView extends Sitengine_View {
			...
			if(is_dir($dir))
			{
				...
				foreach($dirIterator as $count => $file)
				{
					if(is_file($dir.'/'.$file->getFilename()) && !preg_match('/^(\.|\.\.|\.DS_Store)$/', $file->getFilename()))
					{
						...
						$v = (sizeof($markedRows) && isset($markedRows[$id])) ? $this->_controller->getRequest()->getPost($p) : Sitengine_Newsletter_Backend_Campaigns_Attachments_Controller::VALUE_NONESELECTED;
						...
						$type .= Sitengine_Form_Element::getHidden($h, Sitengine_Newsletter_Backend_Campaigns_Attachments_Controller::VALUE_NONESELECTED);
						
						$args = array(
							Sitengine_Env::PARAM_CONTROLLER => 'attachmentsUploads',
							Sitengine_Env::PARAM_FILE => $filename
						);
						$uriDownload  = $this->_controller->getFrontController()->getNewsletterPackage()->getDownloadHandler();
