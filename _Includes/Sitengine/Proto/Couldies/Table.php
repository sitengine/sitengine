...
class Sitengine_Proto_Couldies_Table extends Sitengine_Db_TableWithFiles
{
    ...
    public function __construct(array $config = array())
    {
    	if(
    		...
    		$this->_name = $this->_protoPackage->getCouldiesTableName();
    		$this->_primary = 'id';
			...
			$this->_configs[self::FILE1ORIGINAL_ID] = array(
				'dir' => $this->_protoPackage->getCouldyFile1OriginalDir(),
				'mode' => 0644
			);
			$this->_configs[self::FILE1THUMBNAIL_ID] = array(
				'dir' => $this->_protoPackage->getCouldyFile1ThumbnailDir(),
				'mode' => 0644,
				'length' => 100,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			$this->_configs[self::FILE2ORIGINAL_ID] = array(
				'dir' => $this->_protoPackage->getCouldyFile2OriginalDir(),
				'mode' => 0644
			);
			$this->_configs[self::FILE2THUMBNAIL_ID] = array(
				'dir' => $this->_protoPackage->getCouldyFile2ThumbnailDir(),
				'mode' => 0644,
				'length' => 100,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			throw new Sitengine_Proto_Exception('couldies table class init error');
		}
    }
    ...
    public function complementRow(Sitengine_Proto_Couldies_Row $row)
    {
		$data = $row->toArray();
		$data = $this->_complementFileData($data, self::FILE1ORIGINAL_ID, $this->_protoPackage->getCouldyFile1OriginalDir());
		$data = $this->_complementFileData($data, self::FILE1THUMBNAIL_ID, $this->_protoPackage->getCouldyFile1ThumbnailDir());
		$data = $this->_complementFileData($data, self::FILE1FITTED_ID, $this->_protoPackage->getCouldyFile1FittedDir());
		$data = $this->_complementFileData($data, self::FILE2ORIGINAL_ID, $this->_protoPackage->getCouldyFile2OriginalDir());
    ...
    protected function _complementFileData(array $data, $fileId, $dir)
    {
		...
		{
			$args = array(
				Sitengine_Env::PARAM_CONTROLLER => 'couldies',
	...
	public function delete($where)
    {
    	try {
    		$deleted = 0;
    		$couldies = $this->selectRowsAndFiles($where);
    		
    		foreach($couldies as $couldy)
    		{
				$deleted += $this->deleteRowAndFiles($couldy);
    		}
    		return $deleted;
		}
		catch (Exception $exception) {
			require_once 'Sitengine/Proto/Exception.php';
        	throw new Sitengine_Proto_Exception('couldy delete error', $exception);
		}
    }
