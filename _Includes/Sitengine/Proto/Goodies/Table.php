...
class Sitengine_Proto_Goodies_Table extends Sitengine_Db_TableWithFiles
{
    ...
    public function __construct(array $config = array())
    {
    	if(
    		...
    		$this->_name = $this->_protoPackage->getGoodiesTableName();
    		$this->_primary = 'id';
			...
			$this->_configs[self::FILE1ORIGINAL_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile1OriginalDir(),
				'mode' => 0644
			);
			$this->_configs[self::FILE1THUMBNAIL_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile1ThumbnailDir(),
				'mode' => 0644,
				'length' => 100,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			$this->_configs[self::FILE1FITTED_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile1FittedDir(),
				'mode' => 0644,
				'length' => 400,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			$this->_configs[self::FILE2ORIGINAL_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile2OriginalDir(),
				'mode' => 0644
			);
			$this->_configs[self::FILE2THUMBNAIL_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile2ThumbnailDir(),
				'mode' => 0644,
				'length' => 100,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			$this->_configs[self::FILE2FITTED_ID] = array(
				'dir' => $this->_protoPackage->getGoodyFile2FittedDir(),
				'mode' => 0644,
				'length' => 400,
				'method' => 'width',
				'jpgQuality' => 100,
			...
			throw new Sitengine_Proto_Exception('goodies table class init error');
		}
    }
    ...
    public function complementRow(Sitengine_Proto_Goodies_Row $row)
    {
		$data = $row->toArray();
		$data = $this->_complementFileData($data, self::FILE1ORIGINAL_ID, $this->_protoPackage->getGoodyFile1OriginalDir());
		$data = $this->_complementFileData($data, self::FILE1THUMBNAIL_ID, $this->_protoPackage->getGoodyFile1ThumbnailDir());
		$data = $this->_complementFileData($data, self::FILE1FITTED_ID, $this->_protoPackage->getGoodyFile1FittedDir());
		$data = $this->_complementFileData($data, self::FILE2ORIGINAL_ID, $this->_protoPackage->getGoodyFile2OriginalDir());
		$data = $this->_complementFileData($data, self::FILE2THUMBNAIL_ID, $this->_protoPackage->getGoodyFile2ThumbnailDir());
		$data = $this->_complementFileData($data, self::FILE2FITTED_ID, $this->_protoPackage->getGoodyFile2FittedDir());
    ...
    protected function _complementFileData(array $data, $fileId, $dir)
    {
		...
		{
			$args = array(
				Sitengine_Env::PARAM_CONTROLLER => 'goodies',
	...
	public function delete($where)
    {
    	try {
    		$deleted = 0;
    		$goodies = $this->selectRowsAndFiles($where);
    		
    		foreach($goodies as $goody)
    		{
				$where = $this->getAdapter()->quoteInto('goodyId = ?', $goody->id);
				$deleted += $this->_protoPackage->getShouldiesTable()->delete($where);
				$deleted += $this->deleteRowAndFiles($goody);
    		}
    		return $deleted;
		}
		catch (Exception $exception) {
			require_once 'Sitengine/Proto/Exception.php';
        	throw new Sitengine_Proto_Exception('goody delete error', $exception);
		}
    }
