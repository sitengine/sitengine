...
abstract class Sitengine_Proto_Backend_Goodies_Shouldies_Modifier
{
    ...
    public function insert()
    {
        try {
            ...
            $goodyId = $this->_controller->getEntity()->getAncestorId();
            ...
            $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->handleInsertUploads($id);
            ...
            throw new Sitengine_Proto_Backend_Goodies_Shouldies_Exception('insert error', $exception);
        }
    }
    ...
    public function update()
    {
        try {
            ...
            $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->handleUpdateUploads($id, $stored);
            # sanitize data
            foreach($fields as $k => $v)
            {
            ...
            require_once 'Sitengine/Proto/Backend/Goodies/Shouldies/Exception.php';
            throw new Sitengine_Proto_Backend_Goodies_Shouldies_Exception('update error', $exception);
        }
    }
    ...
    public function delete($id)
    {
        try {
            ...
            return $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->delete($where);
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Proto/Backend/Goodies/Shouldies/Exception.php';
            throw new Sitengine_Proto_Backend_Goodies_Shouldies_Exception('delete error', $exception);
        }
    }
    ...
    public function assignFromList($filename, array $data)
    {
        try {
        	# sanitize data
            foreach($data as $k => $v) {
                if(!preg_match('/^(type|cdate|mdate|gid|goodyId)$/', $k)) {
                    unset($data[$k]);
                }
            }
			...
			$this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->handleFileImport($id, $sourcePath);
			$data = array_merge($data, $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->getFileData());
			#Sitengine_Debug::print_r($data);
			return $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->insertFileImport($data);
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Proto/Backend/Goodies/Shouldies/Exception.php';
            throw new Sitengine_Proto_Backend_Goodies_Shouldies_Exception('assign from list error', $exception);
        }
    }
