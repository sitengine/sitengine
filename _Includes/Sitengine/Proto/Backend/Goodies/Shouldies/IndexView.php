...
abstract class Sitengine_Proto_Backend_Goodies_Shouldies_IndexView extends Sitengine_View {
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $types = $this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Controller::PARAM_FILTER_BY_TYPE);
            $users = $this->_controller->getPermiso()->getDirectory()->getAllUsers();
            $users = array_merge($this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Controller::PARAM_FILTER_BY_UID), $users);
            $groups = $this->_controller->getPermiso()->getDirectory()->getAllGroups();
            $groups = array_merge($this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Controller::PARAM_FILTER_BY_GID), $groups);
