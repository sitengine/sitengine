...
abstract class Sitengine_Proto_Backend_Goodies_Shouldies_Couldies_IndexView extends Sitengine_View {
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $types = $this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Couldies_Controller::PARAM_FILTER_BY_TYPE);
            $users = $this->_controller->getPermiso()->getDirectory()->getAllUsers();
            $users = array_merge($this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Couldies_Controller::PARAM_FILTER_BY_UID), $users);
            $groups = $this->_controller->getPermiso()->getDirectory()->getAllGroups();
            $groups = array_merge($this->_controller->getDictionary()->getFromFieldvals(Sitengine_Proto_Backend_Goodies_Shouldies_Couldies_Controller::PARAM_FILTER_BY_GID), $groups);
            ...
            foreach($items as $item)
            {
				...
				$row['displayThisSelect'] = array(
					'name' => $name,
					'value' => (sizeof($markedRows) && isset($markedRows[$row['id']])) ? ($this->_controller->getRequest()->getPost($name)) : $row[$n],
					'OPTIONS' => $this->_controller->getDictionary()->getFromFieldvals($n),
					'current' => Sitengine_Form_Element::getHidden($current, $row[$n])
				);
