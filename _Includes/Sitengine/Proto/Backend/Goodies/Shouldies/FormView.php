...
abstract class Sitengine_Proto_Backend_Goodies_Shouldies_FormView extends Sitengine_View {
    ...
    protected function _getMainSection()
    {
        try {
            ...
            if($this->_inputMode == Sitengine_Env::INPUTMODE_UPDATE)
            {
				...
				$stored = $this->_controller->getFrontController()->getProtoPackage()->getShouldiesTable()->complementRow($this->_controller->getEntity()->getRow());
                ...
                $uri = $this->_controller->getRequest()->getBasePath().'/'.$route->assemble($args, true);
                $childActions['couldiesList'] = array(
                    'uri' => $uri,
                    'label' => $this->_controller->getTranslate()->translate('formViewChildActionsSectionCouldiesIndex'),
                    'postfix' => ' ('.$this->_controller->getViewHelper()->countCouldies($stored['id']).')'
                );
            ...
            else
            {
                ...
                if(array_key_exists($n, $input)) { $data[$n] = $input[$n]; }
                    ...
                    $q  = 'SELECT MAX(sorting) AS maxSorting FROM '.$this->_controller->getFrontController()->getProtoPackage()->getShouldiesTableName();
                    $q .= ' WHERE ';#.Sitengine_Permiso::FIELD_OID.' = "'.$this->_controller->getPermiso()->getOrganization()->getId().'"';
                    $q .= ' goodyId = "'.$this->_controller->getEntity()->getAncestorId().'"';
                ...
                $n = 'titleLang0';
                if(array_key_exists($n, $input)) { $data[$n] = $input[$n]; }
                else { $data[$n] = 'Shouldy Record (Language 0/Default)'; }
                
                $n = 'titleLang1';
                if(array_key_exists($n, $input)) { $data[$n] = $input[$n]; }
                else { $data[$n] = 'Shouldy Record (Language 1)'; }
