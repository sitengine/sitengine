...
abstract class Sitengine_Permiso_Backend_Users_IndexView extends Sitengine_View
{
    ...
    public function build()
    {
        try {
            ...
            $this->setSection('USERS', $this->_getMainSection());
            return $this;
        }
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $filter = $table->getFilterInstance(
            	...
            	array(
					#'uid' => Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_UID,
					#'gid' => Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_GID,
					#'type' => Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_TYPE,
					'find' => Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_FIND,
					'reset' => Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_RESET
				),
            ...
            foreach($items as $item)
            {
                ...
                $row['uriMembershipIndex'] = $this->_controller->getRequest()->getBasePath().'/'.$route->assemble($args, true);
                
                $row['isMarked'] = (isset($markedRows[$row['id']])) ? $markedRows[$row['id']] : 0;
                $row['membershipCount'] = $this->_controller->getViewHelper()->countMemberships($row['id']);
            ...
            ########################################################################
            $args = array();
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_BATCH);
			...
			require_once 'Sitengine/Permiso/Backend/Users/Exception.php';
			throw new Sitengine_Permiso_Backend_Users_Exception('list page error', $exception);
		}
    }
}
