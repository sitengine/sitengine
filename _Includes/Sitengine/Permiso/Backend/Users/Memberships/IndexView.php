...
abstract class Sitengine_Permiso_Backend_Users_Memberships_IndexView extends Sitengine_View {
    ...
    public function build()
    {
        try {
            ...
            $this->setSection('MEMBERSHIPS', $this->_getMainSection());
            return $this;
        }
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $filter = $table->getGroupJoinFilterInstance(
            	$this->_controller->getRequest(),
            	array(
					'find' => Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_BY_FIND,
					'reset' => Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_RESET
				),
			...
			$whereClauses = array(
				$this->_controller->getDatabase()->quoteInto("$membershipsTableName.userId = ?", $this->_controller->getEntity()->getAncestorId()),
				"$membershipsTableName.groupId = $groupsTableName.id",
        		$filter->getSql('')
        	);
        	...
        	$select = $table
        		...
        		->from($groupsTableName, array('name'))
            ...
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_MEMBERSHIPS_BATCH);
			...
			require_once 'Sitengine/Permiso/Backend/Users/Memberships/Exception.php';
			throw new Sitengine_Permiso_Backend_Users_Memberships_Exception('list page error', $exception);
		}
    }
