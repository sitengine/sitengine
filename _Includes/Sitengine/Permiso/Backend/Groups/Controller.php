...
abstract class Sitengine_Permiso_Backend_Groups_Controller extends Sitengine_Controller_Action
{
    ...
    )
    {
        try {
			...
			$this->_templateIndexView = $this->getEnv()->getIncludesDir().'/Sitengine/Permiso/Backend/Groups/_Templates/IndexView.html';
			$this->_templateFormView = $this->getEnv()->getIncludesDir().'/Sitengine/Permiso/Backend/Groups/_Templates/FormView.html';
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
            throw new Sitengine_Permiso_Backend_Groups_Exception('object instantiation error', $exception);
        }
    }
    ...
    protected function _mapInvokeArgs(array $invokeArgs)
    {
    	...
    	)
    	{
    		...
    		require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
    		throw new Sitengine_Permiso_Backend_Groups_Exception('invalid invoke args');
    	}
    }
    ...
    {
    	$config = $config->{$this->getFrontController()->getConfigName()}->backend->groupsController;
		...
		)
		{
			...
			require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
			throw new Sitengine_Permiso_Backend_Groups_Exception('action controller config error');
		}
    }
    */
    ...
    protected function _getTranslateInstance()
    {
		...
		$en = array(
			...
			$this->getEnv()->getIncludesDir().'/Sitengine/Permiso/Backend/Groups/_Dictionary/en.xml',
    ...
    protected function _start()
    {
    	try {
            ...
            require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
            throw new Sitengine_Permiso_Backend_Groups_Exception('init error', $exception);
        }
    }
    ...
    protected function _goToAction($action)
    {
    	...
    	if(is_callable(array($this, $handler))) {
    		...
    		require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
    		throw new Sitengine_Permiso_Backend_Groups_Exception('trying to forward to a non-existing action handler');
    	}
    }
    ...
    protected function _getRestMappings()
    {
    	return array(
    		...
    		Sitengine_Permiso_Backend_Front::ROUTE_GROUPS => array(
    			Sitengine_Env::METHOD_GET => self::ACTION_INDEX
    		),
    		Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_NEW => array(
    			Sitengine_Env::METHOD_GET => self::ACTION_INSERT,
    			Sitengine_Env::METHOD_POST => self::ACTION_DOINSERT
    		),
    		Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_BATCH => array(
    			Sitengine_Env::METHOD_GET => self::ACTION_INDEX,
    			Sitengine_Env::METHOD_PUT => self::ACTION_DOBATCHUPDATE,
    			Sitengine_Env::METHOD_DELETE => self::ACTION_DOBATCHDELETE
    		),
    		Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_SHARP => array(
    ...
    public function restMapperAction()
    {
    	...
    	{
    		require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
    		$exception = new Sitengine_Permiso_Backend_Groups_Exception(
    ...
    protected function _doUpdateAction()
    {
        try {
            ...
            if(!Sitengine_Http::checkReferer()) {
                require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
                throw new Sitengine_Permiso_Backend_Groups_Exception('bad request', Sitengine_Env::ERROR_BAD_REQUEST);
            }
            /*
            $tables = array(
                $this->getFrontController()->getPermiso()->getGroupsTableName() => 'WRITE'
            );
            # lock tables
            ...
            */
			...
			$route = $this->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_SHARP);
    ...
    protected function _doInsertAction()
    {
        try {
			...
			$route = $this->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_GROUPS);
    ...
    protected function _doBatchDeleteAction()
    {
        try {
            ...
            $tables = array(
                ...
                $this->getFrontController()->getPermiso()->getCouldiesTableName() => 'WRITE'
            );
            */
            if(sizeof($rows) > 0) {
            	/*
