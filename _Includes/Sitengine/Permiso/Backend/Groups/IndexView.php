...
abstract class Sitengine_Permiso_Backend_Groups_IndexView extends Sitengine_View
{
    ...
    public function build()
    {
        try {
            ...
            $this->setSection('GROUPS', $this->_getMainSection());
            return $this;
        }
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $filter = $table->getFilterInstance(
            	...
            	array(
					'find' => Sitengine_Permiso_Backend_Groups_Controller::PARAM_FILTER_BY_FIND,
					'reset' => Sitengine_Permiso_Backend_Groups_Controller::PARAM_FILTER_RESET
				),
            ...
            foreach($items as $item)
            {
                ...
                $row['uriMemberIndex'] = $this->_controller->getRequest()->getBasePath().'/'.$route->assemble($args, true);
                
                $row['isMarked'] = (isset($markedRows[$row['id']])) ? $markedRows[$row['id']] : 0;
                $row['memberCount'] = $this->_controller->getViewHelper()->countMembers($row['id']);
            ...
            ########################################################################
            $args = array();
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_BATCH);
			...
			require_once 'Sitengine/Permiso/Backend/Groups/Exception.php';
			throw new Sitengine_Permiso_Backend_Groups_Exception('list page error', $exception);
		}
    }
}
