...
abstract class Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller extends Sitengine_Controller_Action
{
    ...
    )
    {
        try {
			...
			$this->_templateIndexView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Comments/_Templates/IndexView.html';
			$this->_templateFormView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Comments/_Templates/FormView.html';
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
            throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('object instantiation error', $exception);
        }
    }
    ...
    protected function _mapInvokeArgs(array $invokeArgs)
    {
    	...
    	)
    	{
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('invalid invoke args');
    	}
    }
    ...
    {
    	/*
    	$config = $config->{$this->getFrontController()->getConfigName()}->backend->blogsPostsCommentsController;
		...
		)
		{
			...
			require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
			throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('action controller config error');
		}
		*/
    }
    ...
    protected function _getTranslateInstance()
    {
		...
		$en = array(
			...
			$this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Comments/_Dictionary/en.xml'
		);
    ...
    protected function _goToAction($action)
    {
    	...
    	if(is_callable(array($this, $handler))) {
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('trying to forward to a non-existing action handler');
    	}
    }
    ...
    public function factoryAction()
    {
    	...
    	switch($routeName)
    	{
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS_NEW:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS_BATCH:
    		{
    			switch($method) {
    				case Sitengine_Env::METHOD_GET: $action = self::ACTION_INDEX; break;
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS_SHARP:
    		{
    			switch($method) {
    ...
    protected function _doUpdateAction()
    {
        try {
            ...
            if(!Sitengine_Http::checkReferer()) {
                require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
                throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('bad request', Sitengine_Env::ERROR_BAD_REQUEST);
            }
        ...
        try {
            ...
            */
            ...
            if(is_null($data)) {
                ...
                $route = $this->getFrontController()->getRouter()->getRoute(Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS);
