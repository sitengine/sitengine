...
abstract class Sitengine_Blog_Backend_Blogs_Posts_Comments_IndexView extends Sitengine_View {
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $filter = $table->getFilterInstance(
            	...
            	array(
					'find' => Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_BY_FIND,
					'reset' => Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_RESET
				),
				$this->_controller->getNamespace(),
				$this->_controller->getPermiso()->getUsersTableName()
            );
            ...
            $e = new Sitengine_Form_Element(Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_BY_FIND, $filter->getVal(Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_BY_FIND));
            $e->setId('filter'.Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_BY_FIND);
            $e->setClass('filterText');
            $filter->setElement(Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_BY_FIND, $e->getText(20));
            ...
            $query = array(
                Sitengine_Env::PARAM_SORT => $sorting->getActiveRule(),
                Sitengine_Env::PARAM_ORDER => $sorting->getActiveOrder(),
                Sitengine_Blog_Backend_Blogs_Posts_Comments_Controller::PARAM_FILTER_RESET => 1
            );
        	...
        	$select = $table
        		...
        		->from($table)
        		->joinLeft($usersTableName, "$name.uid = $usersTableName.id", array('name', 'firstname', 'lastname', 'nickname'))
			...
			$select = $table
        		...
        		->joinLeft($usersTableName, "$name.uid = $usersTableName.id", array('id'))
        	;
        	foreach($whereClauses as $clause)
        	{
        		if($clause) { $select->where($clause); }
            ...
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_COMMENTS_BATCH);
			...
			require_once 'Sitengine/Blog/Backend/Blogs/Posts/Comments/Exception.php';
			throw new Sitengine_Blog_Backend_Blogs_Posts_Comments_Exception('list page error', $exception);
		}
    }
