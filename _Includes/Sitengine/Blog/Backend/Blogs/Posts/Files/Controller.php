...
abstract class Sitengine_Blog_Backend_Blogs_Posts_Files_Controller extends Sitengine_Controller_Action
{
    ...
    */
    ...
    )
    {
        try {
			...
			if($routeName == Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_UPLOAD) {
			...
			$this->_templateIndexView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Files/_Templates/IndexView.html';
			$this->_templateFormView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Files/_Templates/FormView.html';
			$this->_templateUploadView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Files/_Templates/UploadView.html';
			#$this->_templateAssignView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Files/_Templates/AssignView.html';
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
            throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('object instantiation error', $exception);
        }
    }
    ...
    protected function _mapInvokeArgs(array $invokeArgs)
    {
    	...
    	)
    	{
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('invalid invoke args');
    	}
    }
    ...
    {
    	$config = $config->{$this->getFrontController()->getConfigName()}->backend->blogsPostsFilesController;
		...
		)
		{
			...
			require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
			throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('action controller config error');
		}
    }
    ...
    protected function _getDictionaryInstance()
    {
        ...
        $dictionary->addFiles(
            ...
            array(
				...
				$this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/Files/_Dictionary/en.xml'
			)
        );
        return $dictionary;
    }
    ...
    protected function _start()
    {
    	try {
			...
			if(!$this->_started)
			{
				...
				if($this->getFrontController()->getBlogPackage()->getBlogSlug() !== null)
				{
					$this->getRequest()->setParam(
						Sitengine_Env::PARAM_GREATANCESTORID,
            ...
            require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
            throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('init error', $exception);
        }
    }
    ...
    protected function _goToAction($action)
    {
    	...
    	if(is_callable(array($this, $handler))) {
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('trying to forward to a non-existing action handler');
    	}
    }
    ...
    public function factoryAction()
    {
    	...
    	switch($routeName)
    	{
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_NEW:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_BATCH:
    		{
    			switch($method) {
    				case Sitengine_Env::METHOD_GET: $action = self::ACTION_INDEX; break;
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_SHARP:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_UPLOAD:
    		{
    			switch($method) {
    		...
    		/*
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES_ASSIGN:
    		{
    			switch($method) {
    ...
    protected function _startEntity()
    {
		...
		if(
			...
			require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
			throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('extra files are not supported on '.$type.' posts');
		}
		return true;
    }
    ...
    protected function _doUpdateAction()
    {
        try {
            ...
            if(!Sitengine_Http::checkReferer()) {
                require_once 'Sitengine/Blog/Backend/Blogs/Posts/Files/Exception.php';
                throw new Sitengine_Blog_Backend_Blogs_Posts_Files_Exception('bad request', Sitengine_Env::ERROR_BAD_REQUEST);
            }
        ...
        try {
            ...
            */
            ...
            if(is_null($data)) {
                ...
                $route = $this->getFrontController()->getRouter()->getRoute(Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_FILES);
    ...
    protected function _doUploadAction()
    {
        try {
			...
			print $this->_getModifierModelInstance()->uploadInsert();
            exit;
        }
        catch (Exception $exception) {
            throw $this->_prepareErrorHandler($exception);
    ...
    protected function _doUploadAction()
    {
        try {
            ...
            if(!isset($_FILES['Filedata'])) {
            	...
            	print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_ERROR);
            	exit;
			}
			...
			if(file_exists($path) )
			{
				print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_EXISTS);
				exit;
			}
			else if(is_uploaded_file($_FILES['Filedata']['tmp_name']))
			{
				...
				switch($_FILES['Filedata']['error']) {
					...
					case 1:
						...
						print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_SIZEEXCEEDED);
						exit;
					case 2:
					...
					case 3:
						...
						print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_INCOMPLETE);
						exit;
					case 4:
						print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_NOFILE);
						exit;
					default:
						print $this->getDictionary()->getFromStatus(Sitengine_Env::STATUS_UPLOAD_ERROR);
						exit;
