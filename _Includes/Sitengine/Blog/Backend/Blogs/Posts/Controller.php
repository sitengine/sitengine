...
abstract class Sitengine_Blog_Backend_Blogs_Posts_Controller extends Sitengine_Controller_Action
{
    ...
    protected $_templateFormTextView = null;
    protected $_templateFormPhotoView = null;
    protected $_templateFormGalleryView = null;
    protected $_templateFormQuoteView = null;
    protected $_templateFormLinkView = null;
    protected $_templateFormAudioView = null;
    protected $_templateFormVideoView = null;
    ...
    )
    {
        try {
			...
			$this->_templateIndexView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/IndexView.html';
			$this->_templateFormTextView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormTextView.html';
			$this->_templateFormPhotoView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormPhotoView.html';
			$this->_templateFormGalleryView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormGalleryView.html';
			$this->_templateFormQuoteView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormQuoteView.html';
			$this->_templateFormLinkView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormLinkView.html';
			$this->_templateFormAudioView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormAudioView.html';
			$this->_templateFormVideoView = $this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Templates/FormVideoView.html';
			
			require_once 'Sitengine/Blog/Posts/Table.php';
        }
        catch (Exception $exception) {
            require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
            throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('object instantiation error', $exception);
        }
    }
    ...
    protected function _mapInvokeArgs(array $invokeArgs)
    {
    	...
    	)
    	{
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('invalid invoke args');
    	}
    }
    ...
    {
    	$config = $config->{$this->getFrontController()->getConfigName()}->backend->blogsPostsController;
		...
		)
		{
			...
			require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
			throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('action controller config error');
		}
    }
    ...
    abstract protected function _getFormTextViewInstance();
    abstract protected function _getFormPhotoViewInstance();
    abstract protected function _getFormGalleryViewInstance();
    abstract protected function _getFormQuoteViewInstance();
    abstract protected function _getFormLinkViewInstance();
    abstract protected function _getFormAudioViewInstance();
    abstract protected function _getFormVideoViewInstance();
    ...
    protected function _getTranslateInstance()
    {
		...
		$en = array(
			...
			$this->getEnv()->getIncludesDir().'/Sitengine/Blog/Backend/Blogs/Posts/_Dictionary/en.xml'
		);
    ...
    protected function _start()
    {
    	try {
			...
			if(!$this->_started)
			{
				...
				if($this->getFrontController()->getBlogPackage()->getBlogSlug() !== null)
				{
					$this->getRequest()->setParam(
						Sitengine_Env::PARAM_ANCESTORID,
						$this->getFrontController()->getBlogPackage()->getBlogSlug()
					);
				}
			}
		}
            ...
            require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
            throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('init error', $exception);
        }
    }
    ...
    protected function _goToAction($action)
    {
    	...
    	if(is_callable(array($this, $handler))) {
    		...
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
    		throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('trying to forward to a non-existing action handler');
    	}
    }
    ...
    public function restMapperAction()
    {
    	...
    	switch($routeName)
    	{
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS:
    		{
    			switch($method) {
    				case Sitengine_Env::METHOD_GET: $action = self::ACTION_INDEX; break;
    			}
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_NEW:
    		{
    			switch($method) {
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_BATCH:
    		{
    			switch($method) {
    				case Sitengine_Env::METHOD_GET: $action = self::ACTION_INDEX; break;
    		...
    		case Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS_SHARP:
    		{
    			switch($method) {
    	...
    	if($action === null) {
    		require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
    		$exception = new Sitengine_Blog_Backend_Blogs_Posts_Exception(
    ...
    protected function _doUpdateAction()
    {
        try {
            ...
            if(!Sitengine_Http::checkReferer()) {
                require_once 'Sitengine/Blog/Backend/Blogs/Posts/Exception.php';
                throw new Sitengine_Blog_Backend_Blogs_Posts_Exception('bad request', Sitengine_Env::ERROR_BAD_REQUEST);
            }
            /*
            $tables = array(
        ...
        try {
            ...
            */
            ...
            if(is_null($data)) {
                ...
                $path = array(
                    Sitengine_Env::PARAM_ANCESTORID => $this->getEntity()->getAncestorSlug()
                );
                $args = array(
                    Sitengine_Env::PARAM_SORT => 'cdate',
                    Sitengine_Env::PARAM_ORDER => 'desc'
                );
                $route = $this->getFrontController()->getRouter()->getRoute(Sitengine_Blog_Backend_Front::ROUTE_BLOGS_POSTS);
    ...
    protected function _insertAction()
    {
    	try {
			...
			$type = $this->getRequest()->get(Sitengine_Blog::PARAM_TYPE);
			
			if($type == Sitengine_Blog_Posts_Table::TYPE_PHOTO) {
				$view = $this->_getFormPhotoViewInstance();
				$template = $this->_templateFormPhotoView;
			}
			else if($type == Sitengine_Blog_Posts_Table::TYPE_GALLERY) {
				$view = $this->_getFormGalleryViewInstance();
				$template = $this->_templateFormGalleryView;
			}
			else if($type == Sitengine_Blog_Posts_Table::TYPE_QUOTE) {
				$view = $this->_getFormQuoteViewInstance();
				$template = $this->_templateFormQuoteView;
			}
			else if($type == Sitengine_Blog_Posts_Table::TYPE_LINK) {
				$view = $this->_getFormLinkViewInstance();
				$template = $this->_templateFormLinkView;
			}
			else if($type == Sitengine_Blog_Posts_Table::TYPE_AUDIO) {
				$view = $this->_getFormAudioViewInstance();
				$template = $this->_templateFormAudioView;
			}
			else if($type == Sitengine_Blog_Posts_Table::TYPE_VIDEO) {
				$view = $this->_getFormVideoViewInstance();
				$template = $this->_templateFormVideoView;
			}
			else {
