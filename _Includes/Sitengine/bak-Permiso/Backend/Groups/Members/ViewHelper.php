...
abstract class Sitengine_Permiso_Backend_Groups_Members_ViewHelper extends Sitengine_View {
    	...
    	try {
			...
			*/
			...
			$args = array(
				...
				Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Groups_Members_Controller::ACTION_INDEX,
			...
			$args = array(
				#Sitengine_Env::PARAM_ORG => $this->_controller->getPermiso()->getOrganization()->getNameNoDefault(),
				Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Groups_Members_Controller::ACTION_INSERT,
				Sitengine_Env::PARAM_ANCESTORID => $this->_controller->getEntity()->getAncestorId()
			);
			$route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_GROUPS_MEMBERS);
    ...
    public function getFilterInstance()
    {
		...
		$reset = ($this->_controller->getRequest()->get(Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_RESET));
		
		### filter element ###
		if($reset) { $filter->resetSessionVal(Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_BY_FIND, $this->_controller->getNamespace()); }
		$filter->registerSessionVal(
			$this->_controller->getNamespace(),
			$this->_controller->getRequest(),
			Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_BY_FIND,
			Sitengine_Permiso_Backend_Groups_Members_Controller::VALUE_NONESELECTED
		);
		# set clause
		if($filter->getVal(Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_BY_FIND)) {
			$value = $this->_controller->getDatabase()->quote($filter->getVal(Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_BY_FIND));
			$value = preg_replace('/\s+/', ' ', $value);
			$value = preg_replace('/.(.*)./', "$1", trim($value)); # remove quote delimiters
			$clause  = '(';
			$clause .= 'LOWER(u.name) LIKE LOWER("%'.$value.'%")';
			$clause .= ' OR LOWER(u.firstname) LIKE LOWER("%'.$value.'%")';
			$clause .= ' OR LOWER(u.lastname) LIKE LOWER("%'.$value.'%")';
			$clause .= ' OR LOWER(u.nickname) LIKE LOWER("%'.$value.'%")';
			$clause .= ')';
			$filter->setClause(Sitengine_Permiso_Backend_Groups_Members_Controller::PARAM_FILTER_BY_FIND, $clause);
		}
		return $filter;
    }
    ...
    public function getSortingInstance()
    {
		...
		$sorting->addRule('enabled', 'desc', 'u.enabled asc', 'u.enabled desc');
		$sorting->addRule('name', 'asc', 'u.name asc', 'u.name desc');
		$sorting->addRule('nickname', 'asc', 'u.nickname asc', 'u.nickname desc');
		$sorting->addRule('firstname', 'asc', 'u.firstname asc', 'u.firstname desc');
		$sorting->addRule('lastname', 'asc', 'u.lastname asc', 'u.lastname desc');
		$sorting->addRule('lastLogin', 'asc', 'u.lastLogin asc', 'u.lastLogin desc');
    ...
    protected function _makeBreadcrumbsData()
    {
        ...
        $title = $breadcrumbs[$levelIndex]['name'];
