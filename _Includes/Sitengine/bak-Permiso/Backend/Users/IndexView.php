...
abstract class Sitengine_Permiso_Backend_Users_IndexView extends Sitengine_View {
    ...
    public function build()
    {
        try {
            ...
            $this->setSection('USERLIST', $this->_getMainSection());
            return $this;
        }
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $e = new Sitengine_Form_Element(Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_FIND, $filter->getVal(Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_FIND));
            $e->setId('filter'.Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_FIND);
            $e->setClass('filterText');
            $filter->setElement(Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_BY_FIND, $e->getText(20));
            ...
            $args = array(
                ...
                Sitengine_Env::PARAM_SORT => $sorting->getActiveRule(),
                Sitengine_Env::PARAM_ORDER => $sorting->getActiveOrder(),
                Sitengine_Permiso_Backend_Users_Controller::PARAM_FILTER_RESET => 1
            );
			...
			$now = $date->get(Zend_Date::ISO_8601, Sitengine_Env::LANGUAGE_EN);
            ...
            $q .= ' mdate,';
            $q .= ' lastLogin,';
            $q .= ' (lastRequest >= DATE_SUB("'.$now.'",INTERVAL 5 SECOND)) AS onlineNow,';
            $q .= ' name,';
            $q .= ' nickname,';
            $q .= ' firstname,';
            $q .= ' lastname,';
            ...
            $q .= ' FROM';
            $q .= ' '.$this->_controller->getPermiso()->getUsersTableName().' AS u ';
            ...
            foreach($list as $count => $row)
            {
                ...
                if(
                    ...
                    $row['id']==Sitengine_Permiso::UID_GUEST ||
                    $row['id']==Sitengine_Permiso::UID_LOSTFOUND
                ) {
                    $enabled = $e[1];
                    $locked = $l[1];
                }
                ...
                $args = array(
                    #Sitengine_Env::PARAM_ORG => $this->_controller->getPermiso()->getOrganization()->getNameNoDefault(),
                    Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Controller::ACTION_UPDATE,
                ...
                $row['childCount'] = $this->_controller->getViewHelper()->countMemberships($row['id']);
            ...
            ########################################################################
            $args = array(
            	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Controller::ACTION_DOBATCHDELETE
            );
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS);
            $uriDoBatchDelete = $this->_controller->getRequest()->getBasePath().'/'.$route->assemble($args, true);
            
            $args = array(
            	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Controller::ACTION_DOBATCHUPDATE
            );
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS);
