    ...
    protected function _getMainSection()
    {
    	try {
			...
			if($this->_inputMode == Sitengine_Env::INPUTMODE_UPDATE)
			{
				...
				$args = array(
					Sitengine_Env::PARAM_ID => $stored['id'],
                	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Memberships_Controller::ACTION_DOUPDATE
                );
                $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_MEMBERSHIPS);
			...
			else
			{
				...
				$args = array(
					Sitengine_Env::PARAM_ANCESTORID => $this->_controller->getEntity()->getAncestorId(),
                	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Memberships_Controller::ACTION_DOINSERT
                );
                $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_MEMBERSHIPS);
                $submitUri = $this->_controller->getRequest()->getBasePath().'/'.$route->assemble($args, true);
			...
			if($payloads->isMain())
            {
				...
				$groups = array_merge($this->_controller->getDictionary()->getFromFieldvals('select'), $groups);
