...
abstract class Sitengine_Permiso_Backend_Users_Memberships_IndexView extends Sitengine_View {
    ...
    public function build()
    {
        try {
            ...
            $this->setSection('MEMBERSHIPLIST', $this->_getMainSection());
            return $this;
        }
    ...
    protected function _getMainSection()
    {
        try {
            ...
            $e = new Sitengine_Form_Element(Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_BY_FIND, $filter->getVal(Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_BY_FIND));
            $e->setId('filter'.Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_BY_FIND);
            $e->setClass('filterText');
            $filter->setElement(Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_BY_FIND, $e->getText(20));
            ...
            $args = array(
                ...
                Sitengine_Env::PARAM_SORT => $sorting->getActiveRule(),
                Sitengine_Env::PARAM_ORDER => $sorting->getActiveOrder(),
                Sitengine_Permiso_Backend_Users_Memberships_Controller::PARAM_FILTER_RESET => 1
            );
            ...
            $q .= ' FROM';
            $q .= ' '.$this->_controller->getPermiso()->getMembershipsTableName().' AS m,';
            $q .= ' '.$this->_controller->getPermiso()->getGroupsTableName().' AS g';
            
            $q .= ' WHERE';
            $q .= ' m.userId = "'.$this->_controller->getEntity()->getAncestorId().'"';
            $q .= ' AND m.groupId = g.id';
            $q .= $filter->getSql();
            ...
            $args = array(
            	Sitengine_Env::PARAM_ANCESTORID => $this->_controller->getEntity()->getAncestorId(),
            	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Memberships_Controller::ACTION_DOBATCHDELETE
            );
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_MEMBERSHIPS);
            ...
            $args = array(
            	Sitengine_Env::PARAM_ANCESTORID => $this->_controller->getEntity()->getAncestorId(),
            	Sitengine_Env::PARAM_ACTION => Sitengine_Permiso_Backend_Users_Memberships_Controller::ACTION_DOBATCHUPDATE
            );
            $route = $this->_controller->getFrontController()->getRouter()->getRoute(Sitengine_Permiso_Backend_Front::ROUTE_USERS_MEMBERSHIPS);
